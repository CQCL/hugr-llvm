use hugr::{extension::{prelude::{self, BOOL_T, QB_T}, ExtensionId}, ops::{custom::OpaqueOp, CustomOp}, std_extensions::arithmetic::float_types::{self, FLOAT64_TYPE}, type_row, types::FunctionType, HugrView};
    ...
    fn emit(&mut self, args: EmitOpArgs<'c, CustomOp, H>) -> Result<()> {
        ...
        match opaque.name().as_str() {
            "H" => {
                ...
                let h_func = self.0.module().add_function("___h", func_type, Some(Linkage::External));
                ...
                args.outputs.finish(builder, [call.try_as_basic_value().unwrap_left()])?;
                Ok(())
            }
            "RzF64" => {
                let func_type = self.0.llvm_func_type(&FunctionType::new(type_row![QB_T, FLOAT64_TYPE], QB_T))?;
                let h_func = self.0.module().add_function("___rz", func_type, Some(Linkage::External));
                let inputs = args.inputs.into_iter().map_into().collect_vec();
                let builder = self.0.builder();
                let call = builder.build_call(
                    h_func,
            ...
            "QAlloc" => {
                let func_type = self.0.llvm_func_type(&FunctionType::new(type_row![], QB_T))?;
                let h_func = self.0.module().add_function("___qalloc", func_type, Some(Linkage::External));
                let inputs = args.inputs.into_iter().map_into().collect_vec();
                let builder = self.0.builder();
                let call = builder.build_call(
                    h_func,
            ...
            "Measure" => {
                ...
                let func_type = self.0.llvm_func_type(&FunctionType::new(QB_T, type_row![QB_T,BOOL_T]))?;
                let h_func = self.0.module().add_function("___measure", func_type, Some(Linkage::External));
                let inputs = args.inputs.into_iter().map_into().collect_vec();
                let builder = self.0.builder();
                let call = builder.build_call(
                    h_func,
