...
impl<'a, H: HugrView + 'a> CodegenExtsBuilder<'a, H> {
    ...
    /// Register a callback to map a [hugr::CustomType] to a [BasicTypeEnum].
    pub fn custom_type(
        mut self,
    ...
    /// Register a callback to emit a [hugr::ExtensionOp], keyed by fully
    /// qualified [OpName].
    pub fn extension_op(
        mut self,
    ...
    /// Register callbacks to emit [hugr::ExtensionOp]s that match the
